...
export default function CategoryManager() {
  ...
  const cacheKey = `categories_p${currentPage}_s${searchName}`;
  const initialCachedData = getClientCache<any>(cacheKey);

  const [categories, setCategories] = useState<Category[]>(initialCachedData?.items || []);
  const [totalPages, setTotalPages] = useState<number>(initialCachedData?.totalPages || 1);
  const [totalItems, setTotalItems] = useState<number>(initialCachedData?.totalItems || 0);
  ...
  const fetchCategories = useCallback(async (page = 1, search = "") => {
    const key = `categories_p${page}_s${search}`;
    ...
    try {
      const result = await categoryService.fetchCategories(page, itemsPerPage, search);
  ...
  // Fetch categories cleanly on page change or debounced search change
  useEffect(() => {
  ...
  }, [currentPage, searchName, fetchCategories]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const isSuccess = Boolean(result && (result.success || result.Success));
      if (isSuccess) {
        clearClientCache("categories_");
        setIsDeleteModalOpen(false);
        const targetPage = categories.length === 1 && currentPage > 1 ? currentPage - 1 : currentPage;
    ...
    <div className="space-y-6">
                    ...
                    <TableRow key={category.id}>
                      ...
                      <TableCell className="px-5 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Link
                            href={`/dashboard/categories/show/${category.id}`}
      ...
      >
        <div className="mb-6 flex flex-col items-center">
          ...
          <h3 className="mb-2 text-xl font-bold text-gray-900 dark:text-white">Delete Category</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">
